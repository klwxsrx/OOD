...
struct CanvasAdapterFixture
{
	std::string TestAdapter(std::function<void(graphics_lib::ICanvas& canvas)> testFunc)
	{
		std::ostringstream outStream;
		{
			modern_graphics_lib::CModernGraphicsRenderer renderer(outStream);
...
BOOST_FIXTURE_TEST_SUITE(CanvasAdapter, CreatedRendererFixture)

	BOOST_AUTO_TEST_CASE(can_be_created)
	{
		BOOST_CHECK_NO_THROW(CCanvasAdapter adapter(renderer));
	}

	BOOST_AUTO_TEST_CASE(not_throws_error_when_created_with_already_started_to_draw_renderer)
	{
		renderer.BeginDraw();
		CCanvasAdapter* adapter = nullptr;
		BOOST_CHECK_NO_THROW(adapter = &CCanvasAdapter(renderer));
		if (adapter != nullptr)
		{
			adapter->LineTo(100, 150);
			BOOST_CHECK_EQUAL(outStream.str(), R"text(<draw>
