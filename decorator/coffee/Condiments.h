...
{
public:
	std::string GetDescriptionByGrade()const override
	{
		// Описание декорированного напитка добавляется к описанию оборачиваемого напитка
		return m_beverage->GetDescriptionByGrade() + ", " + GetCondimentDescription();
	}
...
protected:
	...
	std::string GetCondimentDescription()const override
	{
		return "Chocolate " + std::to_string(m_quantity) + " slices";
	}
private:
	unsigned m_quantity;
};
...
protected:
	...
	std::string GetCondimentDescription()const override
	{
		std::string stringType = (m_type == LiquorType::Chocolate) ? "Chocolate" : "Nut";
		return  stringType.append(" liquor");
	}
private:
	LiquorType m_type;
};